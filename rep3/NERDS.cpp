...
void solve() {
    ...
    for (int i = 0; i < n; i++) {
        int isNerd, shoeSize, typing;
        cin>>isNerd>>shoeSize>>typing;
        if (isNerd == 1)
            nerd.push_back(vector2(shoeSize, typing));
        else
            other.push_back(vector2(shoeSize, typing));
    }
    
    polygon nerdHull = giftWrap(nerd);
    polygon otherHull = giftWrap(other);
    
    if (polygonIntersects(nerdHull, otherHull)) {
